...
import { MikaSidebarGroupConfig } from '@mikaverse/core';
import { from, switchMap, tap } from 'rxjs';
import { MikaContextService, MikaConfigService, MikaAuthService } from '@mikaverse/core';
...
export class MikaSidebarService {
	...
	async renderSidebar(tenantSidebarGroups?: MikaSidebarGroupConfig[]): Promise<MikaSidebarGroupConfig[]> {
		...
		console.log('Rendering sidebar with settings', settings, tenantSidebarGroups);
		if (!tenantSidebarGroups || tenantSidebarGroups!.length !== 0) {
			tenantSidebarGroups = settings?.sidebarGroups ?? [];
		}

		console.log('Rendering sidebar with settings', tenantSidebarGroups);
		...
		for (const [key, config] of this.context.entityConfigs().entries()) {
			...
			// Check entity-specific SHOW permission if configured; undefined = allow all
			const requiredPermission = resolved.permissions?.requiredClaims?.['show'];
		...
		for (const group of groupsMap.values()) {
			group.items?.sort((a: any, b: any) => (a.order ?? 0) - (b.order ?? 0));
		}
